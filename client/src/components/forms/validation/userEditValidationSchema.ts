...
const userEditValidationSchema = Yup.object({
    ...
    birthdate: Yup.date()
        ...
        .min(new Date(new Date().setFullYear(new Date().getFullYear() - 100)), "BirthDate cannot be more than 100 years ago"),
    city: Yup.string().nullable().min(3, "City should be from 3 to 50 symbols").max(100, "City should be from 3 to 50 symbols"),
    gender: Yup.mixed().oneOf([Genders.Male, Genders.Female], "Gender should be specified").required("Gender should be specified"),
    phone: Yup.string()
